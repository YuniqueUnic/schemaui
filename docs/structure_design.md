...
- **Input sources** – `io::input::parse_document_str` uses `serde_json`,
  `serde_yaml`, and `toml` (feature-gated) to read JSON/TOML/YAML from files,
  stdin, or inline strings. The CLI mirrors this logic via `InputSource`
  detection: it first attempts to open a path, and if the OS returns `NotFound`
  the raw string is parsed directly.
- **Schema + config relationship** – users may provide a canonical schema plus a
  config snapshot. `schema_with_defaults` injects snapshot values as `default`
  keywords without mutating the structural parts of the schema. If only data is
  provided, `schema_from_data_value/str` infers the schema and annotates it with
  defaults automatically.
- **Format hints & features** – `DocumentFormat` exposes keyword/extension
  parsing and `available_formats()` is feature-aware (only returns YAML/TOML
  when the corresponding feature is compiled). This enables build-time
...
- **Outputs** – `io::output::OutputOptions` groups format selection,
  ...
  The CLI accepts multiple `-o/--output` values (including `-` for stdout) and
  falls back to a temp file unless `--no-temp-file` is set. It also checks for
  feature/extension mismatches (e.g., requesting `.yaml` when `yaml` is
  disabled) and existing files unless `--force` is passed.
- **Diagnostics** – the CLI’s `DiagnosticCollector` retains every issue (invalid
  ...
  and reports them together after argument parsing.
...
- **Keymap pipeline** – Introduces `keymap/default.keymap.json`, parsed by
  `app::keymap` once via `std::sync::LazyLock`. Each entry contains:
