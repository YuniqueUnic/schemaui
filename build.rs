...
fn main() {
    ...
    if in_cargo_package_dir(&manifest_dir) {
        if should_build(&web_ui_dir, &dist_index) || !dist_index.exists() {
            panic!(
                "web/dist is missing or stale during cargo publish; run `npm run build` \
                 (or set SCHEMAUI_WEB_FORCE_BUILD=1) before publishing."
            );
        }
        return;
    }
